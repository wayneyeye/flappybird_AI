...
def convpool(X,W,b):
	...
	pool_out=tf.nn.max_pool(conv_out,ksize=[1,4,4,1],strides=[1,2,2,1],padding='SAME')
	return pool_out
...
with tf.name_scope("cnn"):
	...
	# first frame
	with tf.device("/gpu:0"):
		X1=tf.reshape(tf.slice(X,[0,1,0,0,0],[-1,1,-1,-1,-1]),[-1,128,128,3])
...
with tf.name_scope("cnn_output"):
	with tf.device("/gpu:0"):
		...
		Z_diff=tf.contrib.layers.flatten(Z12)

with tf.name_scope("dense_layers"):
	with tf.device("/gpu:1"):
		# fully connected layer
		hidden1=my_dense(Z_diff,n_hidden1)
		hidden1_dropped=tf.layers.dropout(hidden1,rate=0.1,training=training_flag)
		hidden2=my_dense(hidden1_dropped,n_hidden2)
		hidden2_dropped=tf.layers.dropout(hidden2,rate=0.1,training=training_flag)
...
class flappy_agent():
	def __init__(self):
		...
		self.n_max_step=500 # size of replay memory
	...
	def update_model(self):
		...
		if len(self.all_obs)==self.n_max_step: # update once reached enough replay memory
			...
			self.Z_snapshot=self.sess.run([Z_diff],
			feed_dict={X:self.obs_X,q_target:self.target_q_values_array})[0]
			self.W1_filter=self.sess.run([W1],
			feed_dict={X:self.obs_X,q_target:self.target_q_values_array})[0]
			self.W2_filter=self.sess.run([W2],
			feed_dict={X:self.obs_X,q_target:self.target_q_values_array})[0]
